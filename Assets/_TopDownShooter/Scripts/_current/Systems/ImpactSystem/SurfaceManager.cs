        ...
        private void PlayEffects(Vector3 hitPoint, Vector3 hitNormal, SurfaceEffect surfaceEffect, float soundOffset)
        {
            foreach (SpawnObjectEffect spawnObjectEffect in surfaceEffect.SpawnObjectEffects)
            {
                if (spawnObjectEffect.Probability > Random.value)
                {
                    if (!_spawnPool)
                    {
                        _spawnPool = Instantiate(spawnObjectEffect.Prefab, transform).GetComponent<ParticleSystem>();
                    }
                    _spawnPool.gameObject.transform.position = hitPoint + hitNormal * 0.001f;
                    _spawnPool.gameObject.transform.rotation = Quaternion.LookRotation(hitNormal);

                    _spawnPool.transform.forward = hitNormal;
                    if (spawnObjectEffect.RandomizeRotation)
                    {
                        ...
                        _spawnPool.transform.rotation = Quaternion.Euler(transform.rotation.eulerAngles + offset);
                    }
                    _spawnPool.Emit(1);
                }
            }
